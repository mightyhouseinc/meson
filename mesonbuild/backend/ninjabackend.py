...
def get_rsp_threshold() -> int:
    ...
    limit = 8192 if mesonlib.is_windows() else 131072
    # Be conservative
    limit //= 2
...
class NinjaBuildElement:
    ...
    def add_item(self, name: str, elems: T.Union[str, T.List[str, CompilerArgs]]) -> None:
        ...
        if name == 'DEPFILE':
            self.elems.append((f'{name}_UNQUOTED', elems))
    ...
    def write(self, outfile):
        ...
        if implicit_outs:
            implicit_outs = f' | {implicit_outs}'
        use_rspfile = self._should_use_rspfile()
        if use_rspfile:
            rulename = f'{self.rulename}_RSP'
...
class NinjaBackend(backends.Backend):
    ...
    def detect_vs_dep_prefix(self, tempfilename):
        ...
        for compiler in self.environment.coredata.compilers.host.values():
            ...
            return open(tempfilename, 'a', encoding='utf-8')
        filebase = f'incdetect.{compilers.lang_suffixes[compiler.language][0]}'
        ...
        with self.detect_vs_dep_prefix(tempfilename) as outfile:
            ...
            if capture:
                captured_compile_args_per_target = {
                    target.get_id(): self.generate_common_compile_args_per_src_type(
                        target
                    )
    ...
    def get_generated_headers(self, target):
        ...
        for genlist in target.get_generated_sources():
            if isinstance(genlist, (build.CustomTarget, build.CustomTargetIndex)):
                continue
            header_deps.extend(
                self.get_target_generated_dir(target, genlist, src)
